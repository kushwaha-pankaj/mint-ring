...
----------------
1. Ratios 30 / 20 / 50.
   Most tutorials use 70 / 15 / 15. We use 30 / 20 / 50 because we are NOT
   training. The "reference" set just populates the gallery — what matters
   is pose diversity, not raw count. The test set is large (50%) so the
   per-class accuracy numbers have a low statistical noise floor (~2% with
   50 images per class vs ~10% with 10 images).
...
RATIOS = {"reference": 0.30, "val": 0.20, "test": 0.50}
...
for class_dir in sorted(p for p in SRC.iterdir() if p.is_dir()):
    ...
    # this gives approximately 29/19/51 per class.
    n = len(images)
